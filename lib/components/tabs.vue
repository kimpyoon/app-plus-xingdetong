	...
	<view class="tabs" :style="{
		...
		<view>
			<scroll-view scroll-x class="scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="scroll-box" :id="id" :class="{'tabs-scorll-flex': !isScroll}">
					<view class="tab-item xa-line-1" :id="'tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
...
<script>
	 ...
	 * @example <tabs ref="tabs" :list="list" :is-scroll="false"></tabs>
	 */
	...
	export default {
			...
			tabItemStyle() {
				return (index) => {
					...
					if (index == this.currentIndex) {
						...
						style = { ...style, ...this.activeItemStyle };
					} else {
		...
		methods: {
			addUnit(value = 'auto', unit = 'rpx') {
				const numberValidate = function (value) {
					return /^[\+-]?(\d+\.?\d*|\.\d+|\d\.\d+e\+\d+)$/.test(value)
				}
				value = String(value);
				// 用uView内置验证规则中的number判断是否为数值
				return numberValidate(value) ? `${value}${unit}` : value;
			},
			// 设置一个init方法，方便多处调用
			async init() {
				// 获取tabs组件的尺寸信息
