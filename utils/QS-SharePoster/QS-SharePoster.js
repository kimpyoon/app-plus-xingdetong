...
export default function getSharePoster(obj,com) {
			...
			if (imagesArray && imagesArray.length > 0) {
				...
				for (let i = 0; i < imagesArray.length; i++) {
					if (imagesArray[i].url) {
						...
						if (/^http:\/\//.test(imgUrl)) imgUrl = imgUrl.replace("http:", "https:");
...
function drawShareImage(obj,com) { //绘制海报方法
	let {
		type,
		posterCanvasId,
		reserve,
	...
	return new Promise((rs, rj) => {
		...
		let Context = uni.createCanvasContext(posterCanvasId,com);
		if (bgObj && bgObj.path)
